...
def api_update():
    """
    触发数据更新的API（可供网页和其他程序调用）

    请求JSON参数：
    - mode: 'incremental'（默认）、'init'、'range'
    - start: 自定义开始日期（YYYYMMDD，可选）
    - end:   自定义结束日期（YYYYMMDD，可选）
    """
    ...
    try:
        if mode == 'init':
            # 初始化：可选传入start，否则用配置的 INIT_START_DATE
            updater.initialize_data(start_date=start or None)
            msg = '初始化数据任务已完成'
        elif mode == 'range':
            if not (start and end):
                return jsonify({
                    'success': False,
                    'message': '自定义更新需要提供 start 和 end（YYYYMMDD）'
                }), 400
            updater.update_custom_range(start, end)
            msg = f'自定义区间更新完成：{start} ~ {end}'
        else:
            # 默认：增量更新
            updater.update_incremental()
            msg = '增量更新完成'

        # 返回最新数据范围，方便前端或其他程序确认
        min_date, max_date = storage.get_data_date_range()
        return jsonify({
            'success': True,
            'message': msg,
